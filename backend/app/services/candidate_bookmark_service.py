...
class CandidateBookmarkService:
		...
		def bookmark_candidate(candidate_id):
			try:
				...
				prev_bookmark = Bookmark.query.filter_by(employer_id=user.id, candidate_id=candidate_id).first()
				if prev_bookmark is None:
					bookmark = Bookmark(employer_id=user.id, candidate_id=candidate_id)
		...
		def get_bookmarked_candidates():
			try:
				...
				bookmarks = Bookmark.query.filter_by(employer_id=user.id).all()
